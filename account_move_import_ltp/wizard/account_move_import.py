...
class AccountMoveImport(models.TransientModel):
    ...
    def create_invoice_from_attachment(self, xml_root):
        ...
        for inv_head_fields in invoice_header:
            ...
            invoice_payment_due_date_str = inv_head_fields.find(
        ...
        invoice_form, invoice_origin = self._get_invoice_form(
            xml_root,
            invoice_number,
            invoice_date_str,
            invoice_currency,
            invoice_payment_due_date_str,
        )

        move = invoice_form.save()

        move.write({"invoice_origin": invoice_origin})
    ...
    def _get_invoice_form(
        ...
        with Form(
            ...
            invoice_payment_due_date = datetime.datetime.strptime(
                invoice_payment_due_date_str, "%Y-%m-%d"
            ).date()
            ...
            for invoice_line in self._get_dte_lines(xml_root):
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    ...
                    invoice_line_form.account_id = invoice_line.get(
                    ...
                    # (6, 0, [invoice_line.get('tax_ids').id])

                    purchase_references.append(
                        invoice_line.get("purchase_reference").split("-")[0]
                    )
                    po_id = (
                        self.env["purchase.order"]
                        .search([("name", "=", purchase_references[-1].split("-")[0])])
                        .id
                    )
            ...
            # references are defined P0003-1, need to delete
            # the -1 with [:-2] or split it by the - sign
    ...
    def _get_dte_lines(self, xml_root):
        ...
        taxes = self.env["account.tax"].search([("name", "=", "EU purchase")])
        ...
        for invoice_line in xml_invoice_lines[0]:
            ...
            product = self.env["product.product"].search(
                [("product_tmpl_id.item_number", "=", product_code)]
            )
